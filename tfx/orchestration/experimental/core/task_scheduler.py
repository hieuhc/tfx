...
@attr.s(frozen=True)
class TaskSchedulerRequest:
  """Request containing information to invoke a task scheduler.
  ...
  """
  mlmd_handle = attr.ib(type=metadata.Metadata)
  pipeline = attr.ib(type=pipeline_pb2.Pipeline)
  task = attr.ib(type=task_pb2.Task)
  # TODO(goutham): Add resolved input artifacts for the task to execute.


@attr.s(frozen=True)
class TaskSchedulerResponse:
...
class TaskScheduler(abc.ABC):
  ...
  @abc.abstractmethod
  def schedule(self, req: TaskSchedulerRequest) -> TaskSchedulerResponse:
    ...
    Args:
      req: Request to schedule task execution.
    """
...
class TaskSchedulerRegistry:
  ...
  @classmethod
  def create_task_scheduler(cls: Type[T], pipeline: pipeline_pb2.Pipeline,
